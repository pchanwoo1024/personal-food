...
def load_menu():
    ...
    for m in months:
        try:
            r = requests.get(base_url.format(m), timeout=5)
...
if st.button("식단 추천 실행"):
    ...
    # 5) Allergy filter
    filtered = (
        [m for m in menu_list if not any(a in m["name"] for a in allergies)]
        if allergies else menu_list.copy()
    )
    if not filtered:
        st.warning("알레르기 때문에 추천 가능 식단이 없습니다.")
        st.stop()

    # 6) Generate combos and labels
    ...
    st.write(f"- 목표 체중: {target_weight:.1f}kg, 예상 소요: {weeks:.1f}주")
    ...
    for (combo, p), t in zip(ranked, times):
        ...
        st.write(f"{t} → **{items}** ({kc} kcal, 적합도 {p:.2f})")
    st.markdown("### ⏰ 증상별 영양소")
    for s in symptoms:
        for tt, it in symptom_map.get(s, []):
            st.write(f"{tt} → {it}")
    st.markdown("### ⏰ 연령별 영양소")
    for tt, it in age_map:
        st.write(f"{tt} → {it}")
